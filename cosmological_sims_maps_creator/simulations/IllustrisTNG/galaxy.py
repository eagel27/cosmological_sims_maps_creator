...
class IllustrisTNGGalaxy(GalaxyBase):
    def __init__(
        ...
        super(IllustrisTNGGalaxy, self).__init__(should_load_datasets)
    ...
    def calc_systemic_velocity(self):
        ...
        system_stars_vel = np.mean(
            velocities[potential < np.percentile(potential, 5)], axis=0
        )
...
class IllustrisTNGGalaxyMaps(GalaxyMapsBase):
    ...
    def load_catalog_data(self, snapshot):
        ...
        """
        sim_reader = IllustrisTNGSimulationReader(
            BASE_PATH_FIX[snapshot].format(snapshot)
        )
    ...
    def create_galaxy_object(
        ...
        galaxy_reader = IllustrisTNGGalaxyDataReader(
            BASE_PATH_FIX[snapshot].format(snapshot), snapshot, subhalo_id, subhalo_pos
        )
